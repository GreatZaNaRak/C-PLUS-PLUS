...
void getMid(CP::map_bst<int,int>& bst, int start, int last)
{
    if (start == last) {bst[(start+last)/2] = 0; return;}
    int index = (start + last) / 2;
    bst[index] = 0;
    getMid(bst, start, index - 1);
    getMid(bst, index + 1, last);
}

void gen_best_bst(int n,CP::map_bst<int,int> &bst) {
  //write your code here
  return getMid(bst, 1, 7);
